...
func (in *inmem) SetGameStatus(ctx context.Context, id, status string) error {
	...
	if g, ok := in.games[id]; ok {
		g.Status = status
		err := saveGameToFile(in.games[id], in.ticks[id])
		if err != nil {
			fmt.Println("Failed to save game file: " + err.Error())
		}
		return nil
	}
	return ErrNotFound
...
func toArchive(game *pb.Game, frames []*pb.GameTick) gameArchive {
...
func saveGameJSON(id string, json []byte) error {
	fmt.Println("saveGameJSON()")
	err := requireSaveDir()
	if err != nil {
		return err
	}
	...
	fmt.Println("writing...")
	path := "/home/graeme/.battlesnake/games/" + id + ".json"
...
func saveGameToFile(game *pb.Game, ticks []*pb.GameTick) error {
	...
	archive := toArchive(game, ticks)
	json, err := json.MarshalIndent(archive, "", "  ")

	if err != nil {
		return err
	}

	return saveGameJSON(game.ID, json)
}
